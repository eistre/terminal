...
The first is a comparison between the previous Azure deployment described in Chapter~\ref{sec:background-and-previous-system} and the revised Azure deployment described in Chapter~\ref{sec:deployment-environments-and-infrastructure-setup}.
The second is a cross-environment comparison among UT HPC, Microsoft Azure, and Amazon Web Services.
...
The observation period covered February and March 2026.
...
The comparison covers February and March 2026, representing higher- and lower-activity months of the observation period, respectively.
...
Table~\ref{tab:environment-cost-comparison} summarizes the monthly cost comparison across the three deployment environments for February and March 2026.
It also highlights the main cost drivers identified for each environment.

\begin{table}[H]
  \centering
  \caption{Cost comparison of the deployment environments in February and March 2026.}
  ...
  \begin{tblr}{
    ...
    AWS (adjusted) & €19.11 & €19.99 & Lightsail Container, Lightsail database, Elastic Container Service \\
  \end{tblr}
\end{table}
...
However, this conclusion should be approached with caution, as the UT HPC cost estimate is based on pricing rules and monitoring observations rather than a direct provider billing report.
...
At the same time, the session containers are constrained by a relatively fixed \texttt{Dockerfile}-based configuration that creates task-specific files and folders in a predefined way.
