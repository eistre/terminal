...
This chapter establishes the educational and technical background for the thesis.
...
The chapter then outlines the architecture and Azure deployment model of the previous system and identifies the unresolved limitations that define the modernization context for this work.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/figure1-environment}
  \caption{User-facing view of the revised web-based Linux command-line learning environment, showing the task list and browser-based Linux terminal.}
  \label{fig:updated-learning-environment}
\end{figure}

Figure~\ref{fig:updated-learning-environment} shows the revised learning environment with its task list, completed tasks highlighted in green, and a browser-based Linux terminal.

\subsection{Motivation and Prior Work} \label{subsec:motivation-and-prior-work}
Linux command-line knowledge remains important in many systems-oriented computing roles, including system administration, software development, and automation. % TODO do we want to keep these three examples in line with the 1-introduction chapter? I feel like we should use the same 3 examples here and in the intro
...
This thesis builds on that baseline by addressing these limitations while preserving the established educational workflow. % TODO swap "builds on" with something more formal
...
This subchapter outlines the system developed in Eistre's thesis~\cite{eistre_2024}, which serves as the technical starting point for this master's thesis.
...
Azure was chosen partly because it offers educational credits to students, which made it a practical option for an academic project.
...
While the cloud-enabled learning platform developed in Eistre's bachelor's thesis~\cite{eistre_2024} represented a significant advance over the initial prototype, it also exposed limitations related to cost, maintainability, and operational reliability.
This subchapter summarizes the key issues in the baseline system and clarifies why they matter for the modernization and cost-optimization work described in later chapters.
...
\begin{table}[H]
  ...
  \begin{tblr}{
    ...
    September 2025 & €93.12 & Same components as in July, AKS virtual machine compute (€18.45) and its supporting services \\
  \end{tblr}
\end{table}
...
The task is to reduce the platform's always-on footprint while preserving per-user interactive Linux environments for irregular course usage.
...
According to the Lucia maintainer, the project was expected to be deprecated by March 2025 and would transition away from being a maintained, general-purpose authentication library~\cite{pilcrowonpaper_fresh_start_2024}.
...
In addition to the previously described cost and maintainability issues, the previous system exposed several further operational and usability limitations that are relevant to the later redesign. % TODO perhaps swap one "previously" with something else?
First, the environment showed scalability limitations during active course usage.
According to observations reported by the thesis supervisor, some students were occasionally unable to access an interactive shell session because the AKS node pool could not scale due to Azure quota constraints\footnote{Based on private communication with thesis supervisor Alo Peets.}.
This suggests that functional correctness alone did not guarantee reliable operation during active course usage.
...
Third, the authentication approach could be improved from a usability perspective by integrating the University's single sign-on capabilities. % TODO should University be lowercased?
...
Taken together, these observations identify additional redesign pressures related to reliability during active usage, maintainability of multilingual content, and practical access management.
...
These limitations define the modernization context of this thesis and provide the rationale for the revised architecture and implementation choices discussed in the next chapter.
