...
According to the Terraform documentation, Terraform is an infrastructure-as-code (IaC) tool that allows infrastructure to be specified in configuration files and managed through a repeatable workflow~\cite{X}.
...
Terraform supports this by providing input variables, which make it possible to supply configuration values separately from the core infrastructure code~\cite{Y}.
...
% TODO perhaps mention that the entire environment was provisioned in North Europe location as that saw the relative lowest costs at the time of the thesis work (and no need for split location deployments due to quotas)
% TODO we should somehow mention that the maintenance jobs were deployed with ACA jobs
% TODO should we cite ACA here? (\cite{noauthor_azure_nodate)
Similarly to the baseline Azure Deployment, the revised Azure deployment is centered around Azure Container Apps (ACA) as the hosting environment for the main web application as well as the scheduled maintenance jobs.
In this environment, the main Nuxt application runs as a containerized web workload, while the database cleanup and provisioner cleanup functions are deployed separately as scheduled jobs.
% TODO reference actual chapter 3 label
This matches the architectural separation described in Chapter~3, where periodic maintenance was moved out of the main application process.
...
When an interactive terminal session is requested, the application provisions a new user-specific Linux container through Azure Container Instances (ACI). % TODO should cite ACI here https://learn.microsoft.com/en-us/azure/container-instances/container-instances-overview
The use of ACI allows the platform to provide isolated containers for each user without the need to maintain any virtual machines or cluster infrastructure. % TODO cite ACI here https://learn.microsoft.com/en-us/azure/container-instances/container-instances-overview
...
In the model, the runtime cost of the session containers is determined by the requested vCPU and memory resources together with the duration of the session. % TODO cite ACI pricing here https://azure.microsoft.com/en-us/pricing/details/container-instances/

The database layer is implemented using the same Azure Database for MySQL Flexible Server deployment model as in the baseline system using the cheapest available configuration to minimize costs (B1ms burstable tier with 1 vCPU and 2 GB of memory). % TODO cite https://azure.microsoft.com/en-us/pricing/details/mysql/ and eistre_veebipohise_nodate
 % TODO explain that as opposed to a storage account, key vault is used for storing sensitive values such as SSH private keys cite https://learn.microsoft.com/en-us/azure/key-vault/general/overview and eistre_veebipohise_nodate
Similar to the baseline system, email delivery for account verification is handled through Azure Communication Services using an Azure-managed email domain. % TODO cite Azure Communication Services here https://learn.microsoft.com/en-us/azure/communication-services/ and https://learn.microsoft.com/en-us/azure/communication-services/concepts/email/email-overview, and eistre_veebipohise_nodate

% TODO for this chapter, we should note that this environment doesn't support university SSO and instead relies on similar functionality as in the baseline system, where user accounts are managed within the application and email verification is used for account validation. This is a significant difference compared to the UT HPC deployment, where single sign-on was integrated through the university-managed Keycloak instance. Also operational overhead was reduced as the application didn't need to worry about stopping / starting the cluster to optimize costs, which also eliminated an apparent delay for users when the system wasn't already running.
...
The main web application and the scheduled jobs remain deployable as managed container workloads, while the used session containers are provisioned on demand through ACI\@.
