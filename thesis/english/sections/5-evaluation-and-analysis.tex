...
The evaluation focuses on whether the redesign preserved the intended course workflow, lowered the observed Azure baseline cost compared to the earlier deployment, could be deployed across the three target environments, and cleaned up inactive user environments as intended.
...
March 2026 served as a lower-activity follow-up period.
...
The cross-environment comparison should therefore be interpreted as an operational comparison under real course use rather than as a controlled experiment with identical workloads.
...
During the observed period, no major issues affecting the intended workflow were identified.
...
Furthermore, no expired session containers remained in any of the three environments at the end of the evaluation period.
This supports the conclusion that the cleanup mechanisms functioned as intended.
...
Compared with the previous Azure deployment discussed in chapter~\ref{sec:background-and-previous-system}, these figures indicate a significant reduction in the observed always-on cost footprint of the Azure-based hosting model.
...
For that reason, the UT HPC cost data had to be estimated manually using the UT HPC pricing calculator~\cite{center_calculate_nodate} together with the monitoring dashboard described in subchapter~\ref{subsubsec:access-and-namespace-provisioning}.
...
The learning environment was used during actual course activity across all three deployment environments without major issues throughout the evaluation period.
...
This indicates that the redesign addressed the main baseline problem identified in chapter~\ref{sec:background-and-previous-system}, which was the mismatch between the platform's uneven usage pattern and the fixed cost structure of the previous cloud deployment.
...
Within this evaluation, UT HPC appeared to be the strongest fit for long-term institutional use, as it combined the lowest observed cost with university-managed hosting and integration with institutional access management.
...
Within this evaluation, Azure appeared to be the strongest public cloud option because it showed the lowest idle footprint among the public cloud deployments while remaining within a single provider ecosystem.
...
From the perspective of future teaching use, the results of this evaluation suggest that UT HPC is the strongest long-term deployment option when institutional support and integration are available, whereas Azure is the strongest self-contained public cloud deployment option among the environments evaluated.
...
The current evaluation is focused primarily on cost and practical course use.
